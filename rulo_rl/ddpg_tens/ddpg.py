...
def train(sess, env, actor, critic):
    ...
    reward_max_file = csvreader('/home/mtb/rulo_ws/src/rulo_pkg/rulo_rl/ddpg_tens/markers_2017-08-09.csv')
    value = list()
    ...
    for i in xrange(MAX_EPISODES):               
        ...
        for j in xrange(MAX_EP_STEPS):
            ...
            if exp_indice > abs(epsilon):
                a_type = "Exploit"
                a_t =  actor.predict(np.reshape(s_t, (1, 2)))#actor.model.predict(s_t.reshape(1, s_t.shape[0]))*1#rescale
            else:
                a_type = "Explore"
...
#             # Added exploration noise
            # a = actor.predict(np.reshape(s_t, (1, 2))) + (1. / (1. + i))
            # print a_t, a_type

            ob2,r_t,done = env.step(s_t[0],a_t[0] )   
            # print ob, r_t, done
            s_t1 = np.array(ob2)

            # print a_type, s_t1

            replay_buffer.add(np.reshape(s_t, (2,)), np.reshape(a_t, (4,)), r_t,
                              done, np.reshape(s_t1, (2,)))
